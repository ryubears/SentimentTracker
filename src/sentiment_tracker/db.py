...
CREATE TABLE IF NOT EXISTS periods (
  period_ts TEXT PRIMARY KEY, horizon TEXT, agg_score REAL,
...
def connect(path: str) -> sqlite3.Connection:
    ...
    # The adaptive weighting layer was removed: with a uniform floor spread over
    # tens of accounts its weights never separated (2.08%-2.30% across 50), so the
    # adaptive and uniform aggregates were the same number to three decimals.
    if "agg_uniform" in [r[1] for r in con.execute("PRAGMA table_info(periods)")]:
        con.execute("ALTER TABLE periods DROP COLUMN agg_uniform")
    con.execute("DROP TABLE IF EXISTS weight_snapshots")
    con.commit()
    return con
...
def save_period(con, period_ts: str, horizon: str, agg: float, price_now: float,
                signals: dict[str, float], counts: dict[str, int]) -> None:
    con.execute("INSERT OR REPLACE INTO periods (period_ts,horizon,agg_score,price_now) "
                "VALUES (?,?,?,?)", (period_ts, horizon, agg, price_now))
