...
def test_anchor_hour_is_midnight_utc_and_pinned_in_meta():
    con = db.connect(":memory:")
    # Existing rows at another hour don't move the anchor — midnight is canonical.
    con.execute("INSERT INTO periods (period_ts, horizon) VALUES ('2026-07-01T05:00:00+00:00', '1d')")
    assert anchor_hour(con, "1d") == 0
    assert db.get_meta(con, "anchor_hour_1d") == "0" # pinned so it can never drift.

def test_anchor_hour_respects_a_deliberately_pinned_meta_value():
    con = db.connect(":memory:")
    db.set_meta(con, "anchor_hour_1d", "5")
    assert anchor_hour(con, "1d") == 5
    assert anchor_hour(con, "1h") == 0 # other horizon unaffected.
